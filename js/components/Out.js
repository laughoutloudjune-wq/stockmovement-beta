...
export default {
  ...
  setup(props) {
    ...
    // Actions
    const addLine = () => {
      lines.value.push({ name: '', qty: '', note: '', stock: null, stockLoading: false });
    };
    ...
    <div class="space-y-6 pb-24">
      ...
      <section class="glass rounded-2xl p-5 shadow-sm space-y-4">
        <div class="flex justify-between items-center">
          ...
          <button class="text-xs font-bold text-blue-500 bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition-colors">
            📜 History
          </button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="date" v-model="form.date" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-800 outline-none shadow-sm focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
          ...
          <div>
            ...
            <ItemPicker v-model="form.requester" source="REQUESTERS" :placeholder="S.pickAdd" />
          </div>
          <div class="md:col-span-2">
            <label class="block text-xs font-bold text-slate-500 mb-1">{{ S.note }}</label>
            <input v-model="form.note" :placeholder="props.lang==='th'?'หมายเหตุ (ถ้ามี)':'Note (Optional)'" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-800 outline-none shadow-sm focus:ring-2 focus:ring-blue-500" />
          </div>
        </div>
      </section>
        ...
        <div v-for="(line, idx) in lines" :key="idx" class="glass rounded-2xl p-4 shadow-sm relative animate-fade-in-up">
          ...
          <div class="space-y-3 pt-2">
            <div class="grid grid-cols-12 gap-3">
              <div class="col-span-8">
                <ItemPicker 
                  v-model="line.name" 
                  source="MATERIALS" 
                  :placeholder="props.lang==='th'?'ระบุวัสดุ...':'Select material...'"
                  @change="onMaterialSelect(line)"
                />
              </div>
              <div class="col-span-4">
                <input 
                  ...
                  v-model="line.qty" 
                  placeholder="0" 
                  class="w-full bg-white border border-slate-200 rounded-xl px-3 py-3 text-center font-bold text-slate-800 outline-none shadow-sm focus:ring-2 focus:ring-blue-500"
                />
              </div>
            </div>
            ...
            <div class="flex items-center justify-between gap-3">
               <input 
                 v-model="line.note" 
                 :placeholder="props.lang==='th'?'หมายเหตุรายตัว...':'Line note...'" 
                 class="flex-1 bg-transparent border-b border-slate-200 text-sm py-1 focus:border-blue-500 outline-none text-slate-600"
               />
               
               <div class="flex items-center gap-2 shrink-0 h-8">
                  <span class="text-xs text-slate-400 font-bold uppercase">{{ props.lang==='th'?'คงเหลือ':'Stock' }}</span>
                  <div v-if="line.stockLoading" class="animate-spin w-4 h-4 border-2 border-slate-300 border-t-blue-500 rounded-full"></div>
                  <div v-else-if="line.stock" :class="line.stock.color" class="px-2 py-0.5 rounded-lg text-xs font-extrabold shadow-sm">
                    {{ line.stock.val }}
                  </div>
